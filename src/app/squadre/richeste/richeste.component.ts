...
console.log(arr);    return arr;
  ...
  //  let c = Observable.forkJoin(...datiprenotazione.user);
    const c = Observable.concat(datiprenotazione.user).map((users: Observable<Observable<User>>) => users.map((userss: Observable<User>) => userss.map( (usersss: User) => usersss.displayName)) );

...
console.log(arr);    return arr;
  ...
  sendwhatup(number, datiprenotazione: Richieste, user ) {
    ...
    Observable.forkJoin(...datiprenotazione.user).combineAll().subscribe(a => console.log(a)) ;// .map( (flat) => flat).subscribe( (aa) => console.log(aa)) ; // .map(( f: User) => f.displayName ).subscribe( hh => console.log(hh));
    // console.log(c);
